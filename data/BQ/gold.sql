...
CREATE TABLE IF NOT EXISTS `hospitalproject-456516.gold_dataset.provider_charge_summary` (
...
# truncate table
TRUNCATE TABLE `hospitalproject-456516.gold_dataset.provider_charge_summary`;

# insert data
INSERT INTO `hospitalproject-456516.gold_dataset.provider_charge_summary`
SELECT 
    CONCAT(p.firstname, ' ', p.LastName) AS Provider_Name,
    d.Name AS Dept_Name,
    SUM(t.Amount) AS Amount
FROM `hospitalproject-456516.silver_dataset.transactions` t
LEFT JOIN `hospitalproject-456516.silver_dataset.providers` p 
    ON SPLIT(p.ProviderID, "-")[SAFE_OFFSET(1)] = t.ProviderID
LEFT JOIN `hospitalproject-456516.silver_dataset.departments` d 
...
# CREATE TABLE
CREATE TABLE IF NOT EXISTS `hospitalproject-456516.gold_dataset.patient_history` (
    Patient_Key STRING,
    FirstName STRING,
    LastName STRING,
    Gender STRING,
...
# TRUNCATE TABLE
TRUNCATE TABLE `hospitalproject-456516.gold_dataset.patient_history`;

# INSERT DATA
INSERT INTO `hospitalproject-456516.gold_dataset.patient_history`
SELECT 
    p.Patient_Key,
    p.FirstName,
    p.LastName,
    ...
    c.PayorType
FROM `hospitalproject-456516.silver_dataset.patients` p
LEFT JOIN `hospitalproject-456516.silver_dataset.encounters` e 
    ON SPLIT(p.Patient_Key, '-')[OFFSET(0)] || '-' || SPLIT(p.Patient_Key, '-')[OFFSET(1)] = e.PatientID
LEFT JOIN `hospitalproject-456516.silver_dataset.transactions` t 
    ON SPLIT(p.Patient_Key, '-')[OFFSET(0)] || '-' || SPLIT(p.Patient_Key, '-')[OFFSET(1)] = t.PatientID
LEFT JOIN `hospitalproject-456516.silver_dataset.claims` c 
...
# CREATE TABLE
CREATE TABLE IF NOT EXISTS `hospitalproject-456516.gold_dataset.provider_performance` (
    ProviderID STRING,
    FirstName STRING,
    LastName STRING,
...
# TRUNCATE TABLE
TRUNCATE TABLE `hospitalproject-456516.gold_dataset.provider_performance`;

# INSERT DATA
INSERT INTO `hospitalproject-456516.gold_dataset.provider_performance`
SELECT 
    pr.ProviderID,
    pr.FirstName,
    pr.LastName,
...
FROM `hospitalproject-456516.silver_dataset.providers` pr
LEFT JOIN `hospitalproject-456516.silver_dataset.encounters` e 
...
# CREATE TABLE
CREATE TABLE IF NOT EXISTS `hospitalproject-456516.gold_dataset.department_performance` (
    Dept_Id STRING,
...
# TRUNCATE TABLE
TRUNCATE TABLE `hospitalproject-456516.gold_dataset.department_performance`;

# INSERT DATA
INSERT INTO `hospitalproject-456516.gold_dataset.department_performance`
SELECT 
    d.Dept_Id,
...
FROM `hospitalproject-456516.silver_dataset.departments` d
LEFT JOIN `hospitalproject-456516.silver_dataset.encounters` e 
...
CREATE TABLE IF NOT EXISTS `hospitalproject-456516.gold_dataset.financial_metrics` AS
SELECT 
    COUNT(DISTINCT t.Transaction_Key) AS TotalTransactions,
    SUM(t.Amount) AS TotalBilledAmount,
    SUM(t.PaidAmount) AS TotalPaidAmount,
...
CREATE TABLE IF NOT EXISTS `hospitalproject-456516.gold_dataset.payor_performance` AS
SELECT 
    c.PayorID,
    c.PayorType,
    COUNT(DISTINCT c.Claim_Key) AS TotalClaims,
...
FROM `hospitalproject-456516.silver_dataset.claims` c
